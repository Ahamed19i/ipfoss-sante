...
export default function AdminLogin() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      await logAction('LOGIN', 'AUTH', `Admin logged in via Google: ${auth.currentUser?.email}`);
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAction('LOGIN', 'AUTH', `Admin logged in via Email: ${email}`);
      navigate(from, { replace: true });
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      ...
      >
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl border border-gray-100 hover:bg-gray-50 transition-all font-bold text-gray-700 shadow-sm"
        >
